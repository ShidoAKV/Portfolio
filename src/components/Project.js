    ...
    <div>
        {PROJECTS.map((project,index)=>(
          <div key={index} className='mb-8  flex flex-wrap lg:justify-center'>
            ...
            >
            <img src={project.image} alt={project.title} width={150} height={150} onClick={() => window.open(project.location)} style={{cursor:'pointer'}}className='mb-6 rounded ' />
            </motion.div>

          <motion.div whileInView={{opacity:1,x:0}} initial={{opacity:0,x:100}} transition={{duration:1}}
           className='w-full max-w-xl lg:w-3/4'>
            <h6  className='mb-2 font-semibold' onClick={() => window.open(project.location)} style={{cursor:'pointer'}}> {project.title}</h6>  
